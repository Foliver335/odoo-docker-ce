...
class MaterialExitWizard(models.TransientModel):
    _name = "xipp.material.exit.wizard"
    _description = "Wizard Saída de Material"
    ...
    def action_confirm(self):
        mat = self.env['xipp.material'].search([('code','=',self.code)], limit=1)
        if mat.quantity < self.qty:
            raise UserError("Estoque insuficiente para saída.")
